...
/********************************************************/
/*         IntercepterSR    intercepterエージェント     */
/********************************************************/

public class IntercepterSR extends Brain{
  ...
  public void playOn(){
    ...
    */
    ...
    if(super.infoManager.hasBallInfo()){  //  ボール情報をもっている
      ...
      /* for OZ
      if(ballX < 0 && pointDiff <= 0)
	dashPower = Generic.STAMINA_INC_MAX * 2;
      */

      if(ballX < super.stdX - 5)
	super.tmpXY.setPointX(ballX + 5);
      //if(ballX < - 40)
      //   super.tmpXY.setPointX(- 40);
      else if(ballX > Generic.FIELD_X_WIDTH * 0.25)
	super.tmpXY.setPointX(super.stdX + 5);
      else
	super.tmpXY.setPointX(super.stdX);
	  ...
	  // for OZ
	  boolean chase2 = super.infoManager.needToChaseBall(2);
	  boolean chase1 = super.infoManager.needToChaseBall(1);

          if(chase2 && predDistBallPos < 30){
	    if(super.infoManager.getStamina() > staminaThreshold)
	       if(chase1)
	         super.player.chaseBall(100);
	       else
