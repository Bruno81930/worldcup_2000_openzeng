...
void ball_position(int i)
{
	...
	tmp2   = norm(mem.sensor.info.see.obj[i].dir - mem.myinfo.ddir) ;

	if(tmp2 != DIR_ERR) {
		mem.ballinfo.pt = norm(tmp2 + mem.myinfo.hdir) ;
		...
		if(tmp1 != DIST_ERR) {
			...
			if(dist_c != CHNG_ERR && dir_c != CHNG_ERR) {
				...
				mem.ballinfo.vt = norm(tmp1 + mem.myinfo.hdir) ;
...
void goal_dir(int i)
{
	...
	if(mem.sensor.info.see.obj[i].dir != DIR_ERR) {
		mem.goalinfo[tmp][1].pt = mem.sensor.info.see.obj[i].dir + mem.myinfo.hdir ;
...
void goalpost_dir(int i)
{
	...
	mem.goalinfo[team][pos].f_visible = TRUE ;

	if(mem.sensor.info.see.obj[i].dir != DIR_ERR) {
		mem.goalinfo[team][pos].pt = mem.sensor.info.see.obj[i].dir + mem.myinfo.hdir ;
		if(mem.sensor.info.see.obj[i].dist != DIST_ERR) {
...
void init_client(void){
	...
	mem.haichi[ 0].x = -50.5 ;	mem.haichi[ 0].y =   0.0 ;
	mem.haichi[ 1].x = -42.0 ;	mem.haichi[ 1].y =  -8.0 ;
	mem.haichi[ 2].x = -42.0 ;	mem.haichi[ 2].y =   8.0 ;
	mem.haichi[ 3].x = -35.0 ;	mem.haichi[ 3].y =   0.0 ;
	mem.haichi[ 4].x = -30.0 ;	mem.haichi[ 4].y = -15.0 ;
	mem.haichi[ 5].x = -30.0 ;	mem.haichi[ 5].y =  15.0 ;
	mem.haichi[ 6].x = -14.0 ;	mem.haichi[ 6].y = -20.0 ;
	mem.haichi[ 7].x = -20.0 ;	mem.haichi[ 7].y =   0.0 ;
	mem.haichi[ 8].x = -14.0 ;	mem.haichi[ 8].y =  20.0 ;
	mem.haichi[ 9].x =  -8.0 ;	mem.haichi[ 9].y = -10.0 ;
	mem.haichi[10].x =  -8.0 ;	mem.haichi[10].y =  10.0 ;
}
